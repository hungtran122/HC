...
def nn_model(X_num, X_cat, cat_len, cat_cols):
    ...
    # out_cat = Dense(32, activation="relu", kernel_initializer="normal")(out_cat)

    input_num = Input(shape=(X_num.shape[1],), name="numeric")
    # out_num = Dense(32, activation="relu", kernel_initializer="normal")(input_num)
    # out_num = BatchNormalization()(out_num)
    # out_num = Dropout(0.8)(out_num)
...
def train():
    ...
    for fold, (train_index, val_index) in enumerate(skf.split(tr_df, y)):
        ...
        oof_preds[val_index] = model.predict([*X_val_cat, X_val_num], batch_size=BATCH_SIZE)
        print('Fold %2d AUC : %.6f' % (fold + 1, roc_auc_score(y_val, oof_preds[val_index])))
...
def submit():
    ...
    for fold in range(n_folds):
        ...
        pred = model.predict([*X_te_cat, X_te_num], batch_size=BATCH_SIZE)
